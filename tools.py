...
# Tool 구현
#  - BaseTool 상속 해서구현.
#      - name, description 지정. args_schema: 툴에 입력할 데이터 설정==> Pydantic.BaseModel 타입으로 정의
#      - 호출할 기능은 `_run()` 에 재정의한다. (run()을 구현하면 안됨. 이건 상위에 있는 것으로 run()->_run() 호출.)
#  - BaseModel
########################################################

# FlightSearchTool 에서 사용할 tool
## DataClass 로 Pydantic으로 구현. 비행 검색 툴의 input으로 들어간다.
...
class FlightSearchTool(BaseTool):
    ...
    args_schema: Type[BaseModel] = FlightSearchInput # 실제 타입 지정.
...
class NearbyPlacesTool(BaseTool):
    ...
    description: str = "특정 장소명으로 인근의 가볼 만한 곳들(관광지, 맛집 등)의 상세 정보를 추천합니다." # 그냥 가볼 만한 곳. 하면 엉뚱한 장소들도 나올 수있다. (관광지, 맛집) 을 퓨샷으로 추가한다.
