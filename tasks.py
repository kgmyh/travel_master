...
# Task는 Agent에게 할당.
# Task 실행의 선후관계는 Sequential일때 있고 그들사이에 data교환이 일어난다. (Agent간의 선후가 아니다.)

# 1단계: 기본 여행 정보 작성 (travel_info_agent)
initial_travel_plan_task = Task(
    description=
        "다음 고객의 요청을 바탕으로 항공편(왕복), 숙소를 포함한 여행 일정을 작성합니다. "
        "요청: {content}",  # placeholder -> kickoff() 에서 전달될 값
    expected_output="한국어로 작성된 일자별 기본 여행 일정과 비용 초안 (항공편(왕복), 숙소 포함)",  # 필수: 이 task의 결과물에 대한 제약조건 (항공편, 숙소 는 agent의 tool에서 제공하는 값을 바탕으로 작성한 것. 프롬프트를 일반적인 문장으로 작성하면 안된다. tool의 실행 결과, 입력값들 등을 가만해서 작성해야 한다.)
    agent=travel_info_agent,  # 이 태스크를 실행할 Agent
)
...
local_recommendation_task = Task(
    ...
    context=[initial_travel_plan_task]  # 1단계 결과 참조. context prompt
)
